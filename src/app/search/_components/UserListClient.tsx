...
import { HoverPrefetchLink } from "@/components/navigation/HoverPrefetchLink";
...
const UserListClient: React.FC<UserListClientProps> = ({ initialData }) => {
  ...
  const currentSection =
    searchParams?.get("section") || (alphabet.length > 0 ? alphabet[0] : "");
  ...
  const resolvedClerkDisplayName = useMemo(() => {
    ...
    const first =
      typeof clerkUser.firstName === "string" ? clerkUser.firstName.trim() : "";
    const last =
      typeof clerkUser.lastName === "string" ? clerkUser.lastName.trim() : "";
    ...
    if (targetSection && alphabet.includes(targetSection)) {
      const timer = setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          if (observerRef.current) {
            alphabet.forEach((letter) => {
              const currentElement = document.getElementById(`group-${letter}`);
  ...
  const handleLetterClick = useCallback(
    (e: React.MouseEvent<HTMLAnchorElement>, letter: string) => {
      e.preventDefault();
      isInitialLoad.current = false;
      if (settleTimerRef.current !== null) {
        window.clearTimeout(settleTimerRef.current);
    ...
    <div className="w-full">
      ...
      <div className="px-2 sm:px-4 mt-4 sm:mt-6 max-w-3xl mx-auto">
        {alphabet.map((letter) => {
          ...
          return <UserGroup key={letter} letter={letter} users={users} />;
        })}
      </div>
    </div>
  );
