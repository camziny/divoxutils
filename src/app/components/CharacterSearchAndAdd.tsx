          ...
          <div className="space-y-1 max-h-64 overflow-y-auto">
            {searchResults.map((character) => (
              <div
                ...
                className="flex items-center justify-between p-3 bg-gray-800 hover:bg-gray-750 rounded border border-gray-700"
              >
                <div className="flex items-center space-x-3 min-w-0">
                  <input
                    type="checkbox"
                    id={character.character_web_id}
                    checked={selectedCharacters.has(character.character_web_id)}
                    onChange={() => handleToggleCharacter(character.character_web_id)}
                    disabled={isLoading}
                    className="w-4 h-4 text-indigo-600 bg-gray-700 border-gray-500 rounded focus:ring-indigo-500 focus:ring-2 disabled:opacity-50"
                  />
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-xs text-gray-400 bg-gray-700 px-2 py-1 rounded">
                        L{character.heraldLevel}
                      </span>
                    </div>
                    <div className="flex items-center space-x-3 text-xs text-gray-400 mt-1">
                      <span className="truncate">{character.heraldClassName}</span>
                      <span>{formatRealmRankWithLevel(character.heraldRealmPoints)}</span>
                      <span className="truncate">{character.heraldGuildName || "No Guild"}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="flex gap-2 mt-4">
            ...
            >
              {isLoading ? (
                ...
                `Add${hasSelections ? ` (${selectedCharacters.size})` : ""}`
              )}
            </Button>
            ...
            <Button
              ...
              disabled={isLoading}
              variant="bordered"
              className="border-gray-600 text-gray-400 hover:text-gray-300 disabled:opacity-50"
            >
              Clear
            </Button>
          </div>
