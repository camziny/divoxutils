...
const AnimatedShieldIcon: React.FC<{ className?: string; style?: React.CSSProperties }> = ({
  className,
  style,
}) => {
...
const DiamondIcon: React.FC<{ className?: string; style?: React.CSSProperties }> = ({
  className,
  style,
}) => (
  <svg viewBox="0 0 24 24" fill="currentColor" className={className} style={style}>
    ...
    <path
      d="M12 4.12L7.03 6.2 5.24 8.74 12 16.58l6.76-7.84L16.97 6.2 12 4.12z"
      opacity="0.6"
    />
  </svg>
);
...
const TierIcon: React.FC<{ tier: number; size: string }> = ({ tier, size }) => {
  ...
  if (tier === 2) {
    ...
    return (
      <span
        className={`relative inline-flex items-center justify-center ${containerWidth} -mx-2`}
          ...
          {sparks.map((sp, i) => (
            <span
              ...
              style={{
                ...
                background:
                  "linear-gradient(180deg, transparent 0%, rgba(165,180,252,0.9) 40%, rgba(199,210,254,1) 50%, rgba(165,180,252,0.9) 60%, transparent 100%)",
                borderRadius: "1px",
                opacity: 0,
  ...
  if (tier === 3) {
    ...
    return (
      <span
        className={`relative inline-flex items-center justify-center ${containerWidth} -mx-3`}
        style={{ height: 0, overflow: "visible" }}
      >
        <span
          className="absolute inline-flex items-center justify-center"
...
export const supporterNameStyle = (
  tier: number
): React.CSSProperties | undefined => {
  if (tier === 3) {
    return {
      color: "#a5b4fc",
      textShadow: "0 0 8px rgba(129,140,248,0.5), 0 0 16px rgba(129,140,248,0.25)",
    };
  }
  return undefined;
};
...
export const SUPPORTER_TIERS = [
  { tier: 1, label: "Tier 1", threshold: "$1/mo", description: "Star icon next to your name" },
  {
    ...
    label: "Tier 2",
    threshold: "$3/mo",
    description: "Animated shield icon next to your name",
  },
  {
    tier: 3,
    label: "Tier 3",
    threshold: "$5/mo",
    description: "Animated diamond icon with highlighted name",
  },
] as const;
